    ...
    return (
        <div>
            <div className = { style.card_card }>
                <NavLink to={'/recipes/' + id} className={style.card_name} activeClassName={style.card_name_i} target="_BLANK">{ name }</NavLink>
                <NavLink to={'/recipes/' + id} target="_BLANK"><img 
                alt="card imgn" 
                src={ image ? image : notfound }
                className={style.card_img}/></NavLink>
                <h5 className={style.card_dt}>Diet-types:</h5>
                <h6 className={style.card_dts}>{ dietType.length ? `  • ${dietType}  ` : "none"}</h6>
            </div>
        </div>
    )
};
